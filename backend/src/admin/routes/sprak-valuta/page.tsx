...
function SprakPage() {
  ...
  useEffect(() => {
    fetch("/admin/wiki-settings?group=languages", { credentials: "include" }).then((r) => r.json()).then((j) => setCur((j.data && j.data.currencies) || {})).catch(() => {})
  }, [])
  const save = async () => {
    setMsg("Sparar…")
    const r = await fetch("/admin/wiki-settings", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ group: "languages", data: { currencies: cur } }) })
  ...
  return (
    <div style={{ display: "flex", fontFamily: WF }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={card}>
          <div style={{ background: "#f4f4f4", borderBottom: "1px solid #ddd", padding: "10px 16px", fontWeight: 700, fontSize: "14px" }}>🌐 Språk och valuta</div>
