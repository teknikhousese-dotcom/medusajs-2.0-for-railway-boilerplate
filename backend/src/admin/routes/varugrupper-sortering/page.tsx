...
/**
 * Teknikhouse.se — Ordning på varugrupperna (mirror of categories.php?sort=1).
 * Pick a nivå (Toppnivå or a parent group), then drag its varugrupper into the
 * order they should appear in. Saves rank (0 = first) on each category via
 * /admin/wiki-sort. No route-config label → stays out of Medusa's native sidebar.
 */
...
function VarugruppersorteringPage() {
  ...
  const label = (c: Cat) => { const chain = [c.name]; let p = c.parent_category_id; let g = 0; while (p && byId[p] && g++ < 6) { chain.unshift(byId[p].name); p = byId[p].parent_category_id }; return chain.join(" › ") }
  ...
  const save = async () => {
    ...
    try {
      ...
      setMsg(r.ok ? `✔ Ordningen sparad (${r.updated} varugrupper).` : "Fel: " + (r.error || ""))
      await loadAll()
    ...
    <div style={{ background: "#fff", border: "1px solid #ddd", borderRadius: "6px", overflow: "hidden", display: "flex", minHeight: "600px", fontFamily: WF }}>
      ...
      <div style={{ flex: 1, minWidth: 0, padding: "16px 22px", maxWidth: "720px" }}>
        <div style={{ textAlign: "center", marginBottom: "12px" }}>
          ...
          <span style={{ fontSize: "16px", fontWeight: 700, verticalAlign: "middle" }}>HANTERA VARUGRUPPER — ORDNING</span>
        </div>
        <p style={{ fontSize: "12px", color: "#444", margin: "0 0 12px" }}>Välj nivå och dra varugrupperna till önskad ordning. Ordningen styr hur de visas i menyer och listningar. Klicka <b>Spara ordning</b> när du är klar.</p>

        <label style={{ fontSize: "12px", fontWeight: 700 }}>Nivå:&nbsp;
          <select style={{ ...inp, minWidth: "320px" }} value={parent} onChange={(e) => setParent(e.target.value)}>
            <option value="root">Toppnivå (huvudvarugrupper)</option>
        ...
        <ul style={{ listStyle: "none", padding: 0, margin: "14px 0", border: "1px solid #ddd", borderRadius: "5px" }}>
          {rows.length === 0 ? <li style={{ padding: "10px", fontSize: "12px", color: "#666" }}>Inga varugrupper på denna nivå.</li> :
            rows.map((c, i) => (
              <li key={c.id}
                draggable
        ...
        <div style={{ textAlign: "center", marginTop: "18px", fontSize: "12px" }}>
          <a href={`${ADMIN}/varugrupper`} style={{ color: "#0060cc" }}>◄ Till Varugrupper</a>
        </div>
      </div>
    </div>
  )
