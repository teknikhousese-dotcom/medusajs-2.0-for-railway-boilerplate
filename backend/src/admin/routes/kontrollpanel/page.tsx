 ...
 * A 1:1 Swedish mirror of the Wikinggruppen "butikadmin" control panel
 * (controls.php): the same left "Snabbmeny" (quick menu) with icons and
 * sub-items, and the same grouped section tiles on the right. Native features
 * link into the live Medusa admin; custom features open an in-hub Swedish page.
 * All data is read same-origin with the logged-in session cookie.
 */
...
type Tile = { emo: string; lab: string; sub?: string; href?: string; section?: string }
type Section = {
...
type MenuItem = { emo: string; lab: string; href?: string; section?: string; subs?: { lab: string; href?: string; section?: string }[] }
...
function KontrollpanelPage() {
  ...
  // Hide Medusa's native left nav while on the Kontrollpanel, so only the
  // Wiki "Snabbmeny" shows — a single, controls.php-style menu. Restored on exit.
  useEffect(() => {
    const findNav = () => Array.from(document.querySelectorAll("div")).find((d) => {
  ...
  const nav = (href?: string, section?: string) => {
    if (href) { window.location.href = href }
    else if (section) { setView(section); window.scrollTo(0, 0) }
  }
  const go = (t: Tile) => nav(t.href, t.section)
  ...
  const menu: MenuItem[] = useMemo(() => ([
    ...
    { emo: "✉️", lab: "Nyhetsbrev", section: "nyhetsbrev", subs: [
      { lab: "Visa prenumeranter (36 969 st)", section: "nyhetsbrev" },
      { lab: "Skriv nytt nyhetsbrev", section: "nyhetsbrev" },
      { lab: "Hantera skickade nyhetsbrev", section: "nyhetsbrev" },
      { lab: "Hantera HTML-mallar", section: "nyhetsbrev" },
    ]},
    { emo: "🤝", lab: "Avtalskunder", href: `${ADMIN}/avtalskunder` },
    { emo: "💰", lab: "Sälj din enhet", href: `${ADMIN}/varderingar` },
    { emo: "🧰", lab: "Hantera produkter", href: `${ADMIN}/hantera-produkter`, subs: [
    ...
    { emo: "🗂️", lab: "Hantera Varugrupper", href: `${ADMIN}/varugrupper`, subs: [
      { lab: "Ny varugrupp", href: `${ADMIN}/varugrupper?action=new` },
      { lab: "Ordning på varugrupperna", href: `${ADMIN}/varugrupper-sortering` },
    ]},
    ...
    { emo: "🔀", lab: "Import / Export", href: `${ADMIN}/hantera-produkter` },
    { emo: "⭐", lab: "Recensioner / Betyg", section: "recensioner" },
    { emo: "🖼️", lab: "Bildspel på 1:a sidan", section: "bildspel" },
    { emo: "📝", lab: "Blogg", section: "blogg" },
    { emo: "↪️", lab: "Hantera gamla URLer", section: "url301" },
    { emo: "🛍️", lab: "Google Shopping", section: "googlefeed" },
  ...
  const groups: { title: string; tiles: Tile[] }[] = useMemo(() => ([
    ...
    { title: "Produkter", tiles: [
      { emo: "➕", lab: "Lägg in ny produkt", sub: "Kopiera produkt", href: `${ADMIN}/produkt-form` },
      { emo: "🧰", lab: "Hantera produkter", href: `${ADMIN}/hantera-produkter` },
      { emo: "🗂️", lab: "Varugrupper", href: `${ADMIN}/varugrupper` },
      { emo: "🔢", lab: "Produktsortering", sub: "Produktfiltrering", href: `${ADMIN}/produktsortering` },
      { emo: "💡", lab: "Rekommendationer", section: "rekommendationer" },
      { emo: "🏷️", lab: "Rabattkoder", href: `${ADMIN}/rabattkoder` },
      { emo: "🚚", lab: "Fraktinställningar", href: `${ADMIN}/fraktinstallningar` },
    ]},
    { title: "Innehåll och inställningar", tiles: [
      ...
      { emo: "📧", lab: "E-postmallar", section: "epost" },
      { emo: "⚙️", lab: "Grundinställningar", href: `${ADMIN}/grundinstallningar` },
      { emo: "↩️", lab: "Returmodul", href: `${ADMIN}/grundinstallningar` },
    ]},
  ...
  const sections: Record<string, Section> = {
    ...
    lager: { key: "lager", emo: "📦", title: "Inköp / Lager", status: "live",
      ...
      actions: [{ lab: "Öppna Lager & Inventering", href: `${ADMIN}/inkop-lager` }],
    ...
    nyhetsbrev: { key: "nyhetsbrev", emo: "✉️", title: "Nyhetsbrev", status: "planering",
      intro: "Din nyhetsbrevslista med 36 969 prenumeranter flyttas hit. Vi rekommenderar att koppla Klaviyo eller Mailchimp för utskick och statistik.",
      bullets: ["Visa prenumeranter (36 969 st)", "Skriv nytt nyhetsbrev", "Hantera skickade nyhetsbrev", "Hantera HTML-mallar"],
  ...
  // Left "Snabbmeny" — faithful to Wiki's controls.php quick menu
  const Snabbmeny = () => (
    <aside className="w-60 shrink-0 border-r bg-ui-bg-subtle rounded-l-lg overflow-hidden">
      ...
      <nav className="py-1 text-[13px]">
        {menu.map((m) => (
          <div key={m.lab}>
            <button onClick={() => nav(m.href, m.section)}
              className="w-full flex items-center gap-2 px-3 py-[7px] text-left hover:bg-ui-bg-base-hover border-b border-ui-border-base/40">
            ...
            {m.subs && m.subs.map((sub) => (
              <button key={sub.lab} onClick={() => nav(sub.href, sub.section)}
                className="w-full text-left pl-10 pr-3 py-[5px] text-[12px] text-ui-fg-subtle hover:bg-ui-bg-base-hover hover:text-ui-fg-base">
                {sub.lab}
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setMenuOpen(false)}
          className="w-full flex items-center gap-2 px-3 py-[7px] text-left hover:bg-ui-bg-base-hover text-ui-fg-muted">
          <span className="text-base leading-none w-5 text-center">⟨⟨</span>Dölj Snabbmenyn
        </button>
      </nav>
    </aside>
  )
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="px-6 pb-8">
        ...
        <p className="text-xs text-ui-fg-muted mt-8 border-t pt-4">
          Speglad 1:1 från Wikinggruppens butikadmin (controls.php) – samma Snabbmeny till vänster och samma sektionsrutor till höger.
        </p>
      </div>
    </div>
  )
    ...
    <div className="bg-ui-bg-base rounded-lg border overflow-hidden flex">
      {menuOpen ? <Snabbmeny /> : (
        <button onClick={() => setMenuOpen(true)}
          className="shrink-0 w-10 border-r bg-ui-bg-subtle hover:bg-ui-bg-base-hover text-ui-fg-muted flex items-start justify-center pt-3" title="Visa Snabbmenyn">⟩⟩</button>
      )}
      {view !== "home" && sections[view] ? <SectionBody /> : <HomeBody />}
    </div>
  )
}
