...
export function installGlobalMenu() {
  ...
  // native /app/orders link to our own order page (/app/ordrar) so it opens the
  // real order list instead of bouncing to the Kontrollpanel.
  try {
    document.addEventListener("click", (ev: any) => {
      try {
        ...
        if (href === "/app/orders" || href.indexOf("/app/orders?") === 0) {
          ev.preventDefault(); ev.stopPropagation()
          window.location.href = "/app/ordrar"
        }
      } catch (e) {}
    }, true)
  } catch (e) {}
