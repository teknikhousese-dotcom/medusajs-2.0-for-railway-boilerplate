...
/**
 * Wiki -> Medusa data bridge (migration only).
 * The old Wiki admin (teknikhouse.se) posts raw product edit-page data here in chunks.
 * Protected by WIKI_BRIDGE_TOKEN (random, migration-only). Rows are stored raw in
 * wiki_product_raw; /admin/wiki-sync applies them. Remove after migration.
 */
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const got = String(req.headers["x-wiki-bridge-token"] || "")
  if (!token || got !== token) return res.status(401).json({ ok: false })
