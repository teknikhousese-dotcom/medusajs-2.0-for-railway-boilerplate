...
/** Idempotently ensure the purchasing tables exist, using the request-scope pg connection. */
export async function ensureTables(scope: any) {
  if (ensured) return
  let pg: any
  try { pg = scope.resolve(ContainerRegistrationKeys.PG_CONNECTION) }
  catch { try { pg = scope.resolve("__pg_connection__") } catch { return } }
