...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const orders = await service.listPurchaseOrders(filter, {
  ...
  let lines: any[] = []
  if (ids.length) {
    lines = await service.listPurchaseOrderLines({ purchase_order_id: ids }, { take: 100000 })
  }
  const byPo: Record<string, any[]> = {}
  for (const l of lines) (byPo[l.purchase_order_id] ||= []).push(l)
  const out = orders.map((o: any) => {
    const ls = byPo[o.id] || []
    const ordered = ls.reduce((s, l) => s + (l.qty_ordered || 0), 0)
    const delivered = ls.reduce((s, l) => s + (l.qty_delivered || 0), 0)
    return { ...o, line_count: ls.length, total_ordered: ordered, total_delivered: delivered }
  })
  res.json({ orders: out })
}
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  if (!supplier_name) {
    try {
      const s = await service.retrieveSupplier(supplier_id)
  ...
  for (const l of lines) {
    ...
    created.push(await service.createPurchaseOrderLines({
      ...
      qty_delivered: 0,
      min_stock: Number(l.min_stock) || 0,
