...
function mapWiki(w: any) {
  ...
  // hide_shop = "Dölj i butiken men inte för sökmotorer (går ej att köpa)": keep the page live
  // (SEO) but make it unbuyable by stocking 0; Wiki's real count stays in metadata.antal.
...
function mapWiki(w: any) {
  ...
  // hide_shop = "Dölj i butiken men inte för sökmotorer (går ej att köpa)": keep the page live
  // (SEO) but make it unbuyable by stocking 0; Wiki's real count stays in metadata.antal.
  const status = visning === "hide_full" ? "draft" : "published"
  const buyable = visning !== "hide_shop"
  const supplier = str(w.supplierID__text)
  const images: any[] = []
  const n = Math.round(num(w.numimages))
