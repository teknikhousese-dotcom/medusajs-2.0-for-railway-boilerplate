...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="py-8 small:py-12">
      <style>{`
        .article-body{color:#2C2F38;font-size:17px;line-height:1.75}
        .article-body p{margin:0 0 1.15em}
        .article-body h2{font-size:1.6rem;font-weight:600;letter-spacing:-.01em;color:#14161C;margin:2em 0 .6em}
      ...
      <div className="content-container">
        <nav className="text-sm text-ui-fg-muted"><Link href="/blogg" className="hover:text-[#F50000]">Blogg</Link><span className="mx-2">/</span><span className="text-ui-fg-subtle">{post.title}</span></nav>
      </div>
      ...
      <article className="content-container mt-6 max-w-3xl">
        ...
        <h1 className="mt-2 text-3xl small:text-4xl font-semibold tracking-tight text-[#14161C] leading-[1.15]">{post.title}</h1>
        <div className="mt-4 flex items-center gap-3 text-sm text-ui-fg-muted">
        ...
        <div className="mt-12 rounded-3xl bg-gradient-to-br from-[#14161C] to-[#232733] p-8 small:p-10 text-white">
          <h2 className="text-2xl font-semibold">Behöver du delar eller hjälp?</h2>
          <p className="mt-2 text-white/75 leading-relaxed max-w-xl">Skärmar, batterier, verktyg och tillbehör med livstidsgaranti på skärmar — eller lämna in din enhet på Phone Rep.</p>
      ...
      {more.length > 0 && (
        <div className="content-container mt-14">
          ...
          <div className="mt-5 grid grid-cols-1 small:grid-cols-3 gap-6">
            {more.map((m) => (
