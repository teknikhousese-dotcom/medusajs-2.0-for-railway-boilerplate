...
const StoreTemplate = ({
  ...
  return (
    <div className="content-container py-6" data-testid="category-container">
      <div className="-mx-6 mb-6 flex gap-2 overflow-x-auto px-6 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden min-[768px]:mx-0 min-[768px]:flex-wrap min-[768px]:px-0">
        {["Mobilreservdelar", "Mobiltillbehör", "Batterier & Powerbank", "Kablar & Laddare", "Hörlurar & Högtalare", "Begagnade mobiler"].map((c) => (
          <a key={c} href={({"Mobilreservdelar":"/mobilreservdelar","Mobiltillbehör":"/mobiltillbehor","Batterier & Powerbank":"/batterier","Kablar & Laddare":"/kablar-laddare","Hörlurar & Högtalare":"/horlurar-hogtalare","Begagnade mobiler":"/mobiler-surfplattor"} as Record<string, string>)[c] || "/store"} className="shrink-0 whitespace-nowrap text-sm font-medium px-4 py-2 rounded-full border border-ui-border-base bg-ui-bg-subtle hover:border-ui-fg-base hover:text-ui-fg-base transition-colors">
            {c}
          </a>
        ))}
      </div>
      <div className="mb-8 border-b border-ui-border-base pb-6">
        <h1 data-testid="store-page-title" className="text-2xl small:text-3xl font-semibold tracking-tight">Alla produkter</h1>
